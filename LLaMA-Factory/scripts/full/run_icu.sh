...
for name in "${names[@]}"
do
    mkdir -p ./logs/RWKU/Target/prompt
    id=$name
    echo $id

    PYTHONPATH=./ WANDB_DISABLED=true python src/train_bash1.py --stage ga \
    --model_name_or_path meta-llama/Meta-Llama-3-8B-Instruct \
    --dataset ${id}_Positive --dataset_dir ./data --use_prompt True \
    --output_dir ./saves/RWKU/Target/${id}/prompt/llama3_8b_instruct --overwrite_cache \
    ...
    --output_result_dir ./results/RWKU/Target/${id}/prompt/llama3_8b_instruct \
    --fp16 --eval_dataset_dir ./data/RWKU/Target/ \
    --target ${id} 2>&1 | tee ./logs/RWKU/Target/prompt/llama3_8b_instruct_${id}.log
done
