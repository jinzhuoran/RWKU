...
for name in "${names[@]}"
do
    mkdir -p ./logs/RWKU/Batch/prompt
    id=$name
    echo $id
    ...
    --output_dir ./saves/RWKU/Batch/${id}/prompt/llama3_8b_instruct --overwrite_cache \
    ...
    --output_result_dir ./results/RWKU/Batch/${id}/prompt/llama3_8b_instruct \
    --fp16 --eval_dataset_dir ./data/RWKU/Batch/ \
    --target ${id} 2>&1 | tee ./logs/RWKU/Batch/prompt/llama3_8b_instruct_${id}.log
done
